...
%TODO: Need altered version of overview graphic
%TODO: Also next slide to talk about formal artifacts
\begin{frame}[t]{Simplex Sandboxing is Essential}
\begin{center}
  \includegraphics[width=4.4in]{img/veriphy-overview.png}
\end{center}
\end{frame}
...
\uncover<1->{\textsf{Live}} &\uncover<1->{\equiv \ddiamond{\prepeat{(\ctrlcolor{\ctrl};\plantcolor{\plant})}}{\textsf{goal}}}\\
\\%
\only<1>{\phantom{\textsf{Win}}  &\phantom{\equiv \ddiamond{\prepeat{(\ctrlcolor{\ctrl};\plantcolor{\plant};\ptest{\textsf{safe}})}}{\textsf{goal}}}}%
\end{align*}
\end{center}
\end{frame}
...
\begin{frame}[t]{\only<1>{Diamonds are Controllers}\only<2>{Games Do It All}}
  \begin{tikzpicture}
    ...
    \draw (0,-0.5) node {$\Uparrow$};
    \draw (0.6,-0.5) node {\footnotesize{\color{gray}Proof}};
    \draw (0,-1) node (nxtprf) {$\textsf{inv}(x) \limply \ddiamond{\ctrlcolor{\ctrl}}{\dbox{\plantcolor{\plant}}{\textsf{inv}(\texttt{--}x)}}$};\pause
    \draw (3, -0.5) node {\plantcolor{dC}};
    \draw (3.15, -1) node {\plantcolor{dI}};
    \draw (3, -1.5) node {\plantcolor{dG}};
    \draw (4.7, -1) node {\plantcolor{\texttt{Plant.S}}};
...
%TODO: Expected more on this, say more while saying less.
%TODO: Describe, what do we actually do with this?
