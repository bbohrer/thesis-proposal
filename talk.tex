...
Automatic   &\cellcolor{yellow!25}paper &\cellcolor{green!25}monitors, controls  &\cellcolor{orange!25}error-prone \\\hline\pause
...
\end{table}
\pause
{\Large What language choices make good tooling possible?}\\\pause
{\Large Thesis statement: \CdGL + Kaisar proofs enable good tools}
\end{frame}

%TODO: Introduce notation at right time.
%Patter: Needs to be games. Needs to be constructive.
\begin{frame}[t]{Tools Should Exploit Proof Content}
  \begin{tikzpicture}
...
\\%
{\small\begin{align*}
\alpha~\equiv~& \prepeat{(\ctrlcolor{\ctrl}; \plantcolor{\plant})}\ \ \ \ctrlcolor{\ctrl} \equiv  (\extCtrlcolor{\extCtrl;} \ctrlcolor{\memnode{asgn}\prandom{\avar}; \memnode{test}\ptest{\admiss}}) \memnode{choice}\cup \fallbackcolor{\fallback}
\end{align*}}\\
\begin{align*}
...
\uncover<2->{\tikz[remember picture,overlay]{
                 \draw[noteline] (choice) -- +(0,-1) node[notebox,text width=1.2cm] {Branch};}}
\uncover<3->{\tikz[remember picture,overlay]{
                 \draw[noteline] (asgn) -- +(-1.4,1.4) node[notebox,text width=2cm] {Instantiate};}}
\uncover<4->{\tikz[remember picture,overlay]{
                 \draw[noteline] (test) -- +(0,+1) node[notebox,text width=2cm] {Demonstrate};}}
\end{frame}
...
\begin{frame}[t]{Box Proofs Are (Only) Monitors}
  \begin{tikzpicture}
    ...
    \draw (0,-0.5) node {$\Uparrow$};
    \draw (0.6,-0.5) node {\footnotesize{\color{gray}Proof}};
    \draw (0,-1) node (nxtprf) {$J(x) \limply \dbox{\plantcolor{\ctrl}}{\dbox{\plantcolor{\plant}}{J(\texttt{--}x)}}$};
    \draw (3, -0.5) node {\plantcolor{dC}};
    \draw (3.15, -1) node {\plantcolor{dI}};
    \draw (3, -1.5) node {\plantcolor{dG}};
...
\tikz[remember picture,overlay]{
                 \draw[noteline] (boxen) -- +(-3,0.3) node[notebox,text width=2.5cm] {All behaviors};}
\tikz[remember picture,overlay]{
                 \draw[noteline] (diamonds) -- +(-3,-0.3) node[notebox,text width=2.5cm] {Some behavior};}
\end{center}
\end{frame}


%Patter: Say the game is between angel (us) and Demon (environment)
\begin{frame}[t]{\only<1-3>{Game Model is Proposed}\only<1-4>{Game Model is Victorious}}
\noindent
\begin{center}
\includegraphics[width=0.4\textwidth]{graphics/fig-ode2.pdf}\\
{\small\begin{align*}
...
\end{align*}}%
\begin{align*}
\uncover<1->{\textsf{Safe}} &\equiv \ddiamond{\memnode{loopz}\drepeat{(\ctrlcolor{\ctrl};\plantcolor{\plant})}}{\textsf{safe}}\\
\uncover<1->{\textsf{Live}} &\equiv \memnode{diamonds}\ddiamond{\prepeat{(\ctrlcolor{\ctrl};\plantcolor{\plant})}}{\textsf{goal}}\\
\uncover<4->{{\textsf{Win}}}  &\uncover<4->{\equiv \ddiamond{\prepeat{(\ctrlcolor{\ctrl};\plantcolor{\plant};\ptest{\textsf{safe}})}}{\textsf{goal}}}%
\end{align*}
\end{center}
\uncover<4->{\tikz[remember picture,overlay]{
                 \draw[noteline] (loopz) -- +(-3,0.3) node[notebox,text width=2.5cm] {Enemy controls loop};}}
\uncover<3->{\tikz[remember picture,overlay]{
                 \draw[noteline] (diamonds) -- +(-3,-0.3) node[notebox,text width=2.5cm] {Player can};}}
\uncover<2->{\tikz[remember picture,overlay]{
...
\begin{frame}[t]{\only<1>{Diamonds are Controllers}\only<2>{Games Do It All}}
  \begin{tikzpicture}
    \draw (0,0) node (mainprf) {$\ddiamond{\prepeat{(\ctrlcolor{\ctrl};\pdual{(\plantcolor{\plant})}; \ptest{\textsf{safe}})}}{\textsf{goal}}$};
    \draw (0,-0.5) node {$\Uparrow$};
    \draw (0.6,-0.5) node {\footnotesize{\color{gray}Proof}};
...
\uncover<1->{\tikz[remember picture,overlay]{
                 \draw[noteline] (asgn) -- +(-1.25,-0.5) node[notebox,text width=1.8cm] {Det.\ assign};}}
...
\begin{textblock}{20}(0.5,3.5)
  \begin{tikzpicture}
    ...
    \draw (0,-1) node (nxtprf) {$J \limply \ddiamond{(\extCtrlcolor{\pdual{\extCtrl;}} \ctrlcolor{(\prandom{\avar})}) \cup \fallbackcolor{\fallback}}{\dbox{\plantcolor{\plant}}{J}}$};
    \draw (-3,-0.5) -- (3,-0.5); \draw (3.5, -0.5) node {$\ctrlcolor{\langle{\cup}\rangle}$};
    \draw (0, 0) node (nxtprf) {$J, \ldots \limply \dbox{\extCtrlcolor{\extCtrl};}{\ddiamond{\ctrlcolor{(\prandom{\avar})}}{\dbox{\plantcolor{\plant}}{J}}}$};
    \draw (-2.5, 0.5) -- (2.5,0.5); \draw (3,0.5) node {$\moncolor{[{?}]}$};
    \draw (0, 1) node (nxtprf) {$J, \ldots \limply \ddiamond{\ctrlcolor{(\prandom{\avar})}}{\dbox{\plantcolor{\plant}}{J}}$};
    \draw (-2, 1.5) -- (2, 1.5); \draw (2.5,1.5) node {$\ctrlcolor{\langle{:}{*}\rangle}$};
    \draw (0,2) node (nxtprf) {$J \limply \dbox{\plantcolor{\plant}}{J}$};
    ...
    \draw (7, 0.5) node {\plantcolor{\texttt{Plant.S}}};

    \draw[color=vblue] (5.2, 1) -- (4.75,0.65);
    \draw[color=vblue] (5.3, 0.5)   -- (4.8,0.5);
    \draw[color=vblue] (5.2, 0) -- (4.75,0.35);

    \draw (-5, 1) node {\ctrlcolor{$\langle{'}\rangle$}};
    \draw (-5, 0.5) node {\ctrlcolor{$\langle{:}{*}\rangle$}};
    \draw (-5, 0) node {\ctrlcolor{$\langle{\cup}\rangle$}};
    \draw (-6, 0.5) node {\ctrlcolor{\texttt{Ctrl.S}}};

    \draw[color=vred] (-4.7, 1) -- (-4.05,0.65);
    \draw[color=vred] (-4.6, 0.5)   -- (-4.1,0.5);
    \draw[color=vred] (-4.7, 0) -- (-4.05,0.35);
  \end{tikzpicture}
\end{textblock}
\end{frame}
%    \draw (0.6,-0.5) node {\footnotesize{\color{gray}Proof}}; 
%    \draw (0.6,-1.5) node {\footnotesize{\color{gray}Proof}};
%\pause  %\ctrlcolor{\ctrl}}{\dbox{\plantcolor{\plant}}{J(y<x)}}$};% \begin{frame}[t]{Systems Driving Model Exists}
% \noindent
...
\node[coordinate,pin={[pin distance=0.9cm,text width=3cm]265:{\ \ \ \ \ \ \ \ \ \S3 \S4}}] (dl-ref) at (dl) {};
\draw (dl) node[circle, fill=blue!20,draw]  {\dL};
\draw (-7, 1.3) node[rectangle,rounded corners,fill=\cback,draw,text depth=1in,text width=1.2in,text centered] (logic) {Logician};
\draw (-7, 2.8) node[rectangle,rounded corners,fill=\cdone,draw,thin,minimum height=0.3cm] (formalization) {\color{vgray}\dL Formalized};
\draw (-7, 1.7) node[rectangle,rounded corners,fill=\cdone,draw,thin,minimum height=0.3cm] (generalization) {\color{vgray}\dL Generalized};
\draw (-14,2.8) node {{\small\color{vgray}Not in Talk}};
\draw (-14,1.7) node {{\small\color{vgray}Not in Talk}};
...
\draw[dashed] (generalization) -- (gen-ref);
%2.6
\draw (5, 1.4) node[rectangle,rounded corners,fill=\cback,draw,text depth=1in,text width=2.1cm,text centered] (engine) {Engineer};
...
\draw node[right=0.4cm of controllers] (controllers-ref) {\S8};
\draw node[right=0.3cm of proofscript] (proofscript-ref) {\S7};
\draw node[right=0.3cm of proofterm] (proofterm-ref) {\S4.1};
\draw[dashed] (monitors) -- (monitor-ref);
\draw[dashed] (controllers) -- (controllers-ref);
...
\draw[dashed] (proofterm) -- (proofterm-ref);
\node[shape=coordinate] (use-anchor) at (0,-3){};
\draw (0, -5.1) node[rectangle,rounded corners,fill=\cback,draw,text depth=1.6cm,text width=2.1in,text centered] (luser) {Logic-User};
