...
%Patter: Warning: left out several papers and details of theoretical proposed stuff. Questions welcome.
\section{Introduction}

%TODO: Andre has citation suggestions.
...
\ah{1}{Application}         & \ah{1}{Models LOC (approx.)}                      & \ah{1}{Real system LOC} \\\hline
\acl{1}{Insulin pump}        & \acl{1}{30~\cite{COBELLI198227}}                   & \acl{1}{35K~\cite{OpenAPS}}  \\ % number of equations in paper
\acl{1}{Commercial airliner} & \acl{1}{84-150~\cite{DBLP:conf/fm/PlatzerC09,DBLP:conf/emsoft/JeanninGKGSZP15}} & \acl{1}{14M~\cite{codebases}} \\  % tangential roundabout study, safe_explicit.kyx
\acl{1}{Modern car}          & \acl{1}{29-150~\cite{DBLP:conf/fm/LoosPN11,DBLP:journals/ral/BohrerTMSP19}}  & \acl{1}{100M~\cite{codebases}} %FM kym3 proof, 
  \end{tabular}
\end{center}
\end{frame}
...
\begin{center}
  \begin{tabular}{lll}
    ...
    \acl{1}{Engineer} & \acl{1}{Logician} & \acl{1}{Logic-User}
  \end{tabular}
\end{center}
\end{frame}
...
%TODO: Better describe: what's least error-prone mean.
%TODO: More symmetric wording: before during after thesis
%TODO: Bracket saying "both are about thesis"
\begin{frame}[t]{No prior work meets all requirements}
\begin{table}[tbh]
  \centering
\begin{tabular}{l|c|c|r}
Approach    & Logician                 & Engineer                               & Logic-User\\\hline
Interactive &\cellcolor{green!25}formal    &\cellcolor{yellow!25}manual effort      &\cellcolor{orange!25}labor-intensive\\\hline\pause
Automatic   &\cellcolor{orange!25}disjoint &\cellcolor{green!25}monitors, controls  &\cellcolor{yellow!25}low-fi \\\hline\pause
\dL (start)   &\cellcolor{orange!25}disjoint &\cellcolor{yellow!25}some monitors      &\cellcolor{yellow!25}unstructured\\\hline\pause       %, less labor-intensive
\dL (current) &\cellcolor{green!25}formal    &\cellcolor{yellow!25}some monitors      &\cellcolor{yellow!25}unstructured\\\hline\pause %, less labor-intensive
\dL (final)   &\cellcolor{green!25}formal    &\cellcolor{green!25}monitors, controls  &\cellcolor{green!25}structured\\\hline       %, less labor-intensive
\end{tabular}
  \caption{Comparison of Verification Approaches}
  \label{tab:approach-comparison}
\end{table}
\pause
{\Large What language choices support practical verification?}\\\pause
{\Large Thesis statement: \CdGL + Kaisar proofs do}
\end{frame}

%TODO: Introduce notation at right time.
%Patter: This is preview of talk body. Constructive + Games Good
\begin{frame}[t]{Preview: Tools Should Exploit Proof Content}
  \begin{tikzpicture}
    \draw (0,0) node (mainprf) {System Safe};
    \draw (0,-0.5) node {$\Uparrow$};
    \draw (0.6,-0.5) node {\footnotesize{\color{gray}Proof}};
    \draw (0,-1) node (nxtprf) {$\textsf{inv} \limply \ddiamond{\ctrlcolor{\ctrl}}{\dbox{\plantcolor{\plant}}{\textsf{inv}}}$};
    \draw (3, -0.5) node {\plantcolor{dC}};
    \draw (3.15, -1) node {\plantcolor{dI}};
    \draw (3, -1.5) node {\plantcolor{dG}};
    \draw (5, -1) node {\plantcolor{\texttt{Monitor.S}}};
...
% Everything here has been done for systems and needs to be done for games
%
% Patter: Define MONITORS and CONTROLS
 \begin{frame}[t]{What is End-to-End Verification?}
\begin{center}
\noindent
% End-to-end means bridge the gap
\only<1>{\includegraphics[width=4.4in]{img/veriphy-anim1.png}}%
% How do we "put the model on the implementation? Simplex monitoring!"
\only<2>{\includegraphics[width=4.4in]{img/veriphy-anim2.png}}%
% What makes us special is that we have proofs all the way through (not disjoint)
\only<3>{\includegraphics[width=4.4in]{img/veriphy-anim3.png}}%
% And there's good separation of concern so that easy things are easy, hard things are possible.
% Engineer only cares about drivers for sensing, actuation, external control.
% For small changes (think new hardware version) they can tweak parameters in the model.
% For big changes, they call the Logic User. But neither of them should have to call the Logician.
% All the proofs tying model to practice are done once or done automatically.
\only<4>{\includegraphics[width=4.4in]{img/veriphy-overview.png}}%
%BIG TRANSITION: We're gonna run through the whole pipeline end-to-end.
% As I do this keep in mind that what's implemented today is for dL (classical, systems).
% For the sake of time, I'm going to compare the existing dL version to the proposed CdGL (constructive, games) version as we go.
% I intend to do a mix of new tools and revisions to existing tools to support this upgrade.
\end{center}
\end{frame}
...
% Patter: Model system as a system
% Patter: Point to example of untrusted CONTROL
...
\\%
{\small\begin{align*}
\alpha~\equiv~& \prepeat{(\ctrlcolor{\ctrl}\memnode{compose}; \plantcolor{\plant})\memnode{repeat}}\ \ \ \ctrlcolor{\ctrl} \equiv  (\extCtrlcolor{\extCtrl;} \ctrlcolor{\prandom{\avar\memnode{asgn}}; \memnode{test}\ptest{\admiss}}) \memnode{choice}\cup \fallbackcolor{\fallback}
\end{align*}}\\
\begin{align*}
...
\end{align*}
\end{center}
\uncover<1->{\tikz[remember picture,overlay]{
                 \draw[noteline] (compose) -- +(-0.8,-1.5) node[notebox,text width=1.6cm] {Sequence};}}
\uncover<2->{\tikz[remember picture,overlay]{
                 \draw[noteline] (repeat) -- +(0.6,-1.3) node[notebox,text width=1.2cm] {Repeat};}}
\uncover<3->{\tikz[remember picture,overlay]{
                 \draw[noteline] (choice) -- +(0,-1.4) node[notebox,text width=1.2cm] {Branch};}}
\uncover<4->{\tikz[remember picture,overlay]{
                 \draw[noteline] (asgn) -- +(-1.6,1.6) node[notebox,text width=2cm] {Instantiate};}}
\uncover<5->{\tikz[remember picture,overlay]{
                 \draw[noteline] (test) -- +(0.6,+1.8) node[notebox,text width=2.2cm] {Demonstrate};}}
\end{frame}
...
% Patter: We have this monitor and proof today, and we have synthesis today, but today's synthesis
% Only gives monitors for controls and plants.
% These do allow an end-to-end safety guarantee, but the system-wide guarantee isn't as strong as
% we'd like. How do we know the car achieves functional objectives? How do we know it reaches its
% goal and doesn't just sit there with the monitor blaring?
\begin{frame}[t]{Box Proofs Are (Only) Monitors}
  \begin{tikzpicture}
    \draw (0,0) node (mainprf) {$\ddiamond{\prepeat{(\plantcolor{\ctrl};\plantcolor{\plant})}}{\textsf{goal}}$};
    \draw (0,-0.5) node {$\Uparrow$};
    \draw (0.6,-0.5) node {\footnotesize{\color{gray}Proof}};
    \draw (0,-1) node (nxtprf) {$\textsf{inv} \limply \dbox{\plantcolor{\ctrl}}{\dbox{\plantcolor{\plant}}{\textsf{inv}}}$};
    \draw (3, -0.5) node {\plantcolor{dC}};
    \draw (3.15, -1) node {\plantcolor{dI}};
    \draw (3, -1.5) node {\plantcolor{dG}};
    \draw (5.2, -1) node {\plantcolor{\texttt{PlantMon.S}}};

    \draw[color=vblue] (3.3, -0.5) -- (3.85,-0.85);
    \draw[color=vblue] (3.4, -1)   -- (3.9,-1);
    \draw[color=vblue] (3.3, -1.5) -- (3.85,-1.15);

    \draw (-3, -0.5) node {\plantcolor{$\langle{'}\rangle$}};
    \draw (-3, -1) node {\plantcolor{$\langle{:}{*}\rangle$}};
    \draw (-3, -1.5) node {\plantcolor{$\langle{\cup}\rangle$}};
    \draw (-5.5, -1) node {\plantcolor{\texttt{ControlMon.S}}};

    \draw[color=vblue] (-3.5, -0.5) -- (-3.85,-0.85);
    \draw[color=vblue] (-3.4, -1)   -- (-3.9,-1);
    \draw[color=vblue] (-3.5, -1.5) -- (-3.85,-1.15);
  \end{tikzpicture}
\end{frame}

%Patter: Liveness says there exists a condition that gets us there.
% We'd still have to trust the plant because you're doomed if physics is broken.
% But liveness helps.
% But dL synthesis from liveness is an open question and there are two reasons that we're going
% to switch to CdGL for this
% 1) We've got two separate safety/liveness theorems here.
% That's definitely a lot of duplicated work for the logic-user and seems inelegant for the Logician
% when they have to use this proof to implement synthesis.
% 2) The proofs don't quite fit what we want for synthesis.
% We know that all behaviors of the system are safe, we can put a box around safety.
% We know some behavior is live, we can put a diamond around liveness.
% But we'd really like to put diamonds around the controller and boxes around the plant
% to capture the idea that we control the plant but someone else controls the environment.
...
\end{align*}
\tikz[remember picture,overlay]{
                 \draw[noteline] (boxen) -- +(-3,0.45) node[notebox,text width=2.5cm] {All behaviors};}
\tikz[remember picture,overlay]{
                 \draw[noteline] (diamonds) -- +(-3,-0.45) node[notebox,text width=2.5cm] {Some behavior};}
\end{center}
\end{frame}


% Patter / transition: That's exactly the idea that we can't express with systems,
% and which made me choose games for the thesis.
% Games say that there's an Angel (us) who lives in the diamond and makes control decisions.
% Patter: Say the game is between angel (us) and Demon (environment).
% System execution is now a game between us and our opponent Demon who lives in the box and controls
% the environment.
\begin{frame}[t]{\only<1-3>{Game Model is Proposed}\only<4->{Game Model is Victorious}}
\noindent
\begin{center}
\includegraphics[width=0.4\textwidth]{graphics/fig-ode1.pdf}\\
{\small\begin{align*}
\alpha~\equiv~& \prepeat{(\ctrlcolor{\ctrl}; \plantcolor{\pdual{(\plant)\memnode{dual}}})}\ \ \ \ctrlcolor{\ctrl} \equiv  (\extCtrlcolor{\pdual{\extCtrl;}} \ctrlcolor{(\prandom{\avar})}) \cup \fallbackcolor{\fallback}
\end{align*}}%
\begin{align*}
\uncover<1->{\textsf{Safe}} &\equiv \ddiamond{\drepeat{(\ctrlcolor{\ctrl};\plantcolor{\pdual{\plant}})\memnode{loopz}}}{\textsf{safe}}\\
\uncover<1->{\textsf{Live}} &\equiv \memnode{diamonds}\ddiamond{\prepeat{(\ctrlcolor{\ctrl};\plantcolor{\pdual{\plant}})}}{\textsf{goal}}\\
\uncover<4->{{\textsf{Win}}}  &\uncover<4->{\equiv \ddiamond{\prepeat{(\ctrlcolor{\ctrl};\plantcolor{\pdual{\plant}};\ptest{\textsf{safe}})}}{\textsf{goal}}}%
\end{align*}
\end{center}
\uncover<4->{\tikz[remember picture,overlay]{
                 \draw[noteline] (loopz) -- +(3,0) node[notebox,text width=3.5cm] {Enemy controls loop};}}
\uncover<3->{\tikz[remember picture,overlay]{
                 \draw[noteline] (diamonds) -- +(-3,-0.3) node[notebox,text width=2.5cm] {Angel modality};}}
\uncover<2->{\tikz[remember picture,overlay]{
                 \draw[noteline] (dual) -- +(-1,1.2) node[notebox,text width=1.5cm] {Switch players};}}
\end{frame}



% Say: Try proofs again, it works now
% Patter: We will look in detail at the systems case which is the done part.
% Patter: Proofs should give us MONITOR and CONTROL. Just monitor is pretty good,
% But it's best to know both are correct
\begin{frame}[t]{\only<1>{Diamonds are Controllers}\only<2>{Games Do It All}}
  \begin{tikzpicture}
    \draw (0,0) node (mainprf) {$\ddiamond{\prepeat{(\ctrlcolor{\ctrl};\pdual{(\plantcolor{\plant})}; \ptest{\ctrlcolor{\textsf{safe}}})}}{\textsf{goal}}$};
    \draw (0,-0.5) node {$\Uparrow$};
    \draw (0.6,-0.5) node {\footnotesize{\color{gray}Proof}};
    \draw (0,-1) node (nxtprf) {$\textsf{inv} \limply \ddiamond{\ctrlcolor{\ctrl}}{\dbox{\plantcolor{\plant}}{\ddiamond{\ptest{\ctrlcolor{\textsf{safe}}}}{\textsf{inv}}}}$};\pause
    \draw (3, -0.5) node {\plantcolor{dC}};
    \draw (3.15, -1) node {\plantcolor{dI}};
    \draw (3, -1.5) node {\plantcolor{dG}};
    \draw (5, -1) node {\plantcolor{\texttt{Monitor.S}}};
...
%Transition: And that's the ideal I expect to achieve for games.
% Now let's pull back to reality and see the current synthesis for systems.
% We take the ctrl-plant model and elaborate it into a sandbox style for simplex.
%Patter: Discuss limitations discovered about arithmetic and code generation
...
& \quad \memnode{asgn}\pumod{\vec{x}}{\vec{x}^+}\\
& \quad \prandom{\vec{x}^+} \\
...
\end{textblock}
\uncover<1->{\tikz[remember picture,overlay]{
                 \draw[noteline] (asgn) -- +(-1.25,0) node[notebox,text width=1.8cm] {Det.\ assign};}}

\end{frame}

  \begin{frame}[t]{System-Wide Behavior is Evaluated}
\begin{center}
\noindent
\only<1>{\includegraphics[width=4.4in]{img/veriphy-roles.png}}%
\end{center}
\end{frame}
...
\begin{frame}[t]{Done: End-to-end for Classical Systems}
\newcommand{\cdone}{green!30}
\newcommand{\cwip}{yellow!30}
\newcommand{\cprop}{orange!30}
\newcommand{\cback}{green!15}
%\begin{center}
\noindent\hskip-0.2in
\begin{tikzpicture}[scale=0.5]
%\draw (0,3.2)  node {Logic for};
%\draw (0,2.4)  node {Hybrid Systems};
\draw (0,0)   node[circle]      (CdGLcirc) {\phantom{\hspace{0.5cm}}};
\draw (0,0)   node[circle]  (dl)  {\phantom{\dL}};
\draw (dl) node[circle, fill=blue!20,draw]  {\dL};
\draw (-7, 1.6) node[rectangle,rounded corners,fill=\cback,draw,text depth=1in,text width=1.2in,text centered] (logic) {\logician[\iconsize]Logician};
...
%\draw[dashed] (cgl) -- (cgl-ref);
\draw (5.2, 2) node[rectangle,rounded corners,fill=\cback,draw,text depth=0.75in,text width=2.3cm,text centered] (engine) {\engineer[\iconsize]Engineer};
\draw (5.2, 2.6) node[rectangle,rounded corners,fill=\cdone,draw,thin,minimum height=0.3cm] (monitors) {Monitors};
\draw node[right=0.6cm of monitors] (monitor-ref) {\S4};
%\draw[dashed] (monitors) -- (monitor-ref);
\node[shape=coordinate] (use-anchor) at (0,-3){};
\draw (0, -5.3) node[rectangle,rounded corners,fill=\cback,draw,text depth=1.6cm,text width=2.1in,text centered] (luser) {\logicuser[\iconsize]Logic-User};
\draw[->,very thick] (logic) -- (CdGLcirc);
\draw[->,very thick] (CdGLcirc) -- (engine);

\draw (-9, -4.5) node[rectangle, text width = 2cm,minimum height=0.5cm,fill=\cdone] {};
\draw (-9, -5.5) node[rectangle, text width = 2cm,minimum height=0.5cm,fill=\cprop] {};
\draw (-9, -6.5) node[rectangle, text width = 2cm,minimum height=0.5cm,fill=\cwip] {};

\draw (-9, -4.5) node {Done};
\draw (-9, -5.5) node {Ongoing};
\draw (-9, -6.5) node {Proposed};
\end{tikzpicture}
\end{frame}
%\draw (5, 1.4) node[rectangle,rounded corners,fill=\cwip,draw,thin,minimum height=0.3cm] (proofscript) {Proof Script};
%\draw node[right=0.22cm of maintainable] (maintain-ref) {\S7};
%\draw node[right=0.32cm of lop] (lop-ref) {\S7};
%\draw[dashed] (lop) -- (lop-ref);
%\draw[dashed] (maintainable) -- (maintain-ref);
%\draw node[right=0.6cm of controllers] (controllers-ref) {\S8};
...
%\draw[dashed] (cdgl) -- (cdgl-ref);
%\draw (5, 0.2) node[rectangle,rounded corners,fill=\cprop,draw,thin,minimum height=0.3cm] (controllers) {Controls};
%\draw (0.1, -5.2) node[rectangle,rounded corners,fill=\cprop,draw,thin,minimum height=0.3cm,text width=4.8cm] (lop)    {Modest Proof Effort};
%\draw (-2.9, -6.53) node[rectangle,rounded corners,fill=\cprop,draw,thin,minimum height=0.3cm,text width=2cm] (modelmistake) {Transparency};
%\draw (2.4, -6.5) node[rectangle,rounded corners,fill=\cprop,draw,thin,minimum height=0.3cm,text width=2.4cm] (maintainable) {Maintainability};

\begin{frame}[t]{Proposed: End-to-end \textbf{and Practical}}
\newcommand{\cdone}{green!30}
%\newcommand{\cwip}{yellow!30}
\newcommand{\cprop}{yellow!30}
\newcommand{\cback}{green!15}
%\begin{center}
\noindent\hskip-0.2in
\begin{tikzpicture}[scale=0.5]
...
\draw (5.2, 2) node[rectangle,rounded corners,fill=\cback,draw,text depth=0.75in,text width=2.3cm,text centered] (engine) {\engineer[\iconsize]Engineer};
\draw (5.2, 2.6) node[rectangle,rounded corners,fill=\cdone,draw,thin,minimum height=0.3cm] (monitors) {Monitors};
\draw (5.2, 1.4) node[rectangle,rounded corners,fill=\cprop,draw,thin,minimum height=0.3cm] (controllers) {Controls};
\draw (5.2, 0.2) node[rectangle,rounded corners,fill=\cprop,draw,thin,minimum height=0.3cm] (transp) {Transparency};
\draw node[right=0.6cm of monitors] (monitor-ref) {\S4};
\draw node[right=0.6cm of controllers] (controllers-ref) {\S8};
\draw node[right=0.6cm of transp] (transp-ref) {\S4};
%\draw[dashed] (monitors) -- (monitor-ref);
%\draw[dashed] (controllers) -- (controllers-ref);
%\draw[dashed] (transp) -- (transp-ref);
\node[shape=coordinate] (use-anchor) at (0,-3){};
\draw (0, -5.3) node[rectangle,rounded corners,fill=\cback,draw,text depth=1.6cm,text width=2.1in,text centered] (luser) {\logicuser[\iconsize]Logic-User};
\draw (0.1, -5.2) node[rectangle,rounded corners,fill=\cprop,draw,thin,minimum height=0.3cm,text width=4.8cm] (lop)    {Modest Proof Effort};
\draw (-2.9, -6.53) node[rectangle,rounded corners,fill=\cprop,draw,thin,minimum height=0.3cm,text width=2cm] (modelmistake) {Transparency};
...
%\draw node[right=0.3cm of proofscript] (proofscript-ref) {\S7};
%\draw[dashed] (proofscript) -- (proofscript-ref);
%\draw (0.3, -5.2) node[rectangle,rounded corners,fill=\cprop,draw,thin,minimum height=0.3cm] (monitors)    {Simplicity};
...
%\draw (5, 2.6) node[rectangle,rounded corners,fill=\cdone,draw,thin,minimum height=0.3cm] (proofterm) {\color{vgray}Proof Term};
%\draw (10, 2.6) node {{\small\color{vgray}Not in Talk}};
...
\begin{frame}{Constructive Games Strengthen Proof Content}
  \begin{tabular}{lll}
    \ah{1}{\engineer} & \ah{1}{\logician} & \ah{1}{\logicuser}\\
    \acl{1}{\sayHappy{Program the proof!}} & \acl{1}{\sayHappy{Curry Howard!}} & \acl{1}{\saySad{Low-level proofs!}}
  \end{tabular}
\end{frame}
...
% What's next: ODE's
\begin{frame}[t]{Proof Content is Code}
\begin{textblock}{20}(0.5,3.5)
  \begin{tikzpicture}
    \draw (0,-2) node (mainprf) {$\ddiamond{\prepeat{(\ctrlcolor{\ctrl};\pdual{(\plantcolor{\plant})})}}{\textsf{goal}}$};
    \draw (-3,-1.5)  -- (3,-1.5); \draw (3.5, -1.5) node {$\ctrlcolor{\langle{;}\rangle}$};
    \draw (0,-1) node (nxtprf) {$\textsf{inv} \limply \ddiamond{(\extCtrlcolor{\pdual{(\extCtrl)}}; \ctrlcolor{(\prandom{\avar})}) \cup \fallbackcolor{\fallback}}{\dbox{\plantcolor{\plant}}{\textsf{inv}}}$};
    \draw (-3,-0.5) -- (3,-0.5); \draw (3.5, -0.5) node {$\ctrlcolor{\langle{\cup}\rangle}$};
    \draw (0, 0) node (nxtprf) {$\textsf{inv}, \ldots \limply \dbox{\extCtrlcolor{\extCtrl};}{\ddiamond{\ctrlcolor{(\prandom{\avar})}}{\dbox{\plantcolor{\plant}}{\textsf{inv}}}}$};
    \draw (-2.5, 0.5) -- (2.5,0.5); \draw (3,0.5) node {$\moncolor{[{?}]}$};
    \draw (0, 1) node (nxtprf) {$\textsf{inv} \limply \ddiamond{\ctrlcolor{(\prandom{\avar})}}{\dbox{\plantcolor{\plant}}{\textsf{inv}}}$};
    \draw (-2, 1.5) -- (2, 1.5); \draw (2.5,1.5) node {$\ctrlcolor{\langle{:}{*}\rangle}$};
    \draw (0,2) node (nxtprf) {$\textsf{inv} \limply \dbox{\plantcolor{\plant}}{\textsf{inv}}$};
    \draw (-1.2,2.5) -- (1.2,2.5); \draw (2.2,2.5) node {\moncolor{dC, dG,$\ldots$}};
    \draw (0,3) node (nxtprf) {*};
    \draw (5.8, 1) node {\plantcolor{dC}};
    \draw (5.8, 0.5) node {\plantcolor{$[{?}]$}};
    \draw (5.8, 0) node {\plantcolor{dG}};
    \draw (7.3, 0.5) node {\plantcolor{\texttt{Monitor.S}}};

    \draw[color=vblue] (5.2, 1) -- (4.75,0.65);
    \draw[color=vblue] (5.3, 0.5)   -- (4.8,0.5);
    \draw[color=vblue] (5.2, 0) -- (4.75,0.35);

    \draw (-4.5, 1) node {\ctrlcolor{$\langle{'}\rangle$}};
    \draw (-4.5, 0.5) node {\ctrlcolor{$\langle{:}{*}\rangle$}};
    \draw (-4.5, 0) node {\ctrlcolor{$\langle{\cup}\rangle$}};
    \draw (-5.8, 0.5) node {\ctrlcolor{\texttt{Control.S}}};

    \draw[color=vred] (-4.2, 1) -- (-3.55,0.65);
    \draw[color=vred] (-4.1, 0.5)   -- (-3.6,0.5);
    \draw[color=vred] (-4.2, 0) -- (-3.55,0.35);
  \end{tikzpicture}
\end{textblock}
\end{frame}
%    \draw (0.6,-0.5) node {\footnotesize{\color{gray}Proof}}; 
%    \draw (0.6,-1.5) node {\footnotesize{\color{gray}Proof}};
%\pause  %\ctrlcolor{\ctrl}}{\dbox{\plantcolor{\plant}}{inv}}$};% \begin{frame}[t]{Systems Driving Model Exists}
% \noindent
...
New proofs are \CdGL, but old tools use \dL. % VeriPhy needs: games vs.\ systems
\begin{enumerate}
\item Formally prove and implement: ``Games $\equiv$ Systems $\mod$ Strategies", or
\item Generalize existing \dL tools to \CdGL
\end{enumerate}
% \pause
...
\begin{center}
  \begin{tikzpicture}
\draw (0,4.5) node[rectangle,draw,thin] (input) {\logicuser[\iconsize]\texttt{Proof.kai}};
\draw (0,3) node[rectangle,draw,thin] (parse) {\logician[\iconsize]{Parsing + Elaboration}};
\draw (0,1.8) node[rectangle,draw,thin] (pcheck) {\logician[\iconsize]{Proofchecking}};

%\draw (-3.5,0.6) node (bref) {\color{vgray}Game Refinement};
%\draw (-3.5,0) node[rectangle,draw,thin] (bexport) {Bellerophon Export};
\draw (-4,0.6) node[rectangle,dashed,draw,thin] (bimport) {\dL export};
\draw (-5,1.4) node {\logician[\iconsize]Generate?};
\draw (-5,0.2) node {\logicuser[\iconsize]Rewrite?};
\draw (0,0.6) node[rectangle,draw,thin] (synth) {\logician[\iconsize]{Interpret/Compile}};
%\draw (0,-1.5) node[rectangle,draw,thin] (interp) {\logician[\iconsize]{Interpreter}};
%\draw (4,-1.5) node[rectangle,draw,thin] (comp) {\logician[\iconsize]{Compiler}};

\draw (0,-1) node[rectangle,draw,thin] (impl) {\engineer[\iconsize]{Software Simulations + Hardware Platforms}};

\draw (1.4,-0.3) node {\color{vgray}FFI, socket, \ldots};

\draw (input) -- (parse);
\draw (parse) -- (pcheck);
\draw (pcheck) -- (synth);
\draw[dashed] (bimport) -- (synth);
\draw[dashed] (impl) -- (synth);

%\draw[thin,dotted] (-2.5,3.8) rectangle (2.5,-0.8) {};
%\draw (3.8,1.5) node {Black box};

%\draw[dashed] (synth) -- (bimport);
...
\item Isabelle software floats: support integrals (slower).
...
\begin{frame}[t]{Thesis Makes the Whole Team Happy}
  \begin{tabular}{lll}
    \ah{1}{\engineer} & \ah{1}{\logician} & \ah{1}{\logicuser}\\
    \acl{1}{\sayHappy{Full synthesis suite!}} & \acl{1}{\sayHappy{Formal foundations!}} & \acl{1}{\sayHappy{Structured proofs!}}
  \end{tabular}
...
\begin{frame}[t]{Dedicated to Those Who Clean This Room}
  \begin{center}
    \includegraphics[width=2in]{img/seiu.jpg}
  \end{center}
\end{frame}
