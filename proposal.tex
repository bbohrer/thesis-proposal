...
Of the available approaches to verification, hybrid systems theorem-proving in differential dynamic logic (\dL) and its generalization differential \emph{game} logic (\dGL) is notable for its strong logical foundations and successful application in a number of case studies using the theorem provers \KeYmaera and \KeYmaeraX.
...
Unlike its classical predecessors \dGL and \dL, a \CdGL proof contains, in the general case, enough information to synthesize both monitors and controllers, which respectively check that the external world meets modeling assumptions and compute model-compliant (thus safe) control choices for the CPS implementation.
The push-button-synthesized monitor itself is proven correct down to machine-code level, with an end-to-end formal guarantee in a theorem prover that the implementation code is safe.
...
Not only does our end-to-end approach provide formal guarantees of runtime safety while presenting a clean linguistic interface to proof, but it also validates environmental assumptions,
providing valuable feedback on the realism of modeling assumptions.
\end{abstract}
