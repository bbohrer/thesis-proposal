...
The deceptively simple way to define semantics for constructive games is simply to reinterpret the definitions given in~\cite{DBLP:journals/tocl/Platzer17} over a constructive set theory.
For example, $\fint{\phi \lor \psi}{} = \fint{\phi}{} \cup \fint{\psi}{},$ but $\cup$ is reinterpreted as constructive union to mirror the constructive $\lor,$ thus for example it is not provable that $\{x \mapsto 0\} \in \fint{x < 0 \lor x \geq 0},$ which is the desired behavior.
Also, the definitions for the dual operator probably ought to change so that $\dstrategyfor[\pdual{\alpha}]{X} = \strategyfor[\alpha]{X}$ and $\strategyfor[\pdual{\alpha}]{X} = \dstrategyfor[\alpha]{X}$ because the dual axiom for games is only valid classically.

There are advantages and disadvantages to this approach.
The advantage is that the definitions are very readable, but with the downsides that a novice reader may misunderstand them as the classical definitions and that because we must work in a constructive metalogic we must be careful that we don't accidentally use any reasoning which is only valid classically.

\subsection{Real Constructive Games, Explicit Kleene}
We can give semantics in a traditional Kleene reachability style, but keep the metalogic (and thus the set theory) classical and so make it explicit in the semantics whenever we require witnesses.
This is perhaps the best choice for our model theoretic semantics because it makes the complexity apparent and is hard to misread or to use the wrong proof principle.
Term semantics are as before.

The formula semantics $\fint{\phi}{}$ denote the unique largest region for which Angel has a strategy to prove $\phi$ (under the assumption that the initial state is somewhere in the region).
\newcommand{\mto}{\rightharpoonup}
\begin{align*}
  \fint{\theta \sim \eta}{} &= \{\om~|~\tint{\theta}{\om} \sim \tint{\eta}{\om}\}\\
  ...
  \fint{\dbox{\alpha}{\phi}} &= \dstrategyfor[\alpha]{\fint{\phi}{}}
\end{align*}
Where $A \mto B$ is the set of computable multi-valued functions from A to B, while $A \to B$ is the set of all computable functions from A to B.
...
However, it is important to support multi-valued functions somewhere, because
\begin{inparaenum}
\item The approximate splitting principle used for case analysis on reals will need multivalued functions for most practical proofs.
\item The generality of multivalued functions justifies us implementing a nondeterministic randomized operational semantics.
\end{inparaenum}
...
We can also give a realizability semantics in terms of realizer functions which implement strategies for Angel and Demon, respectively.
These semantics can be silly to read at first, but should make it easy to show that provable formulas correspond to winning strategies, and provide an easy way to give an operational semantics for strategies.
...
We should then formulate and prove the relation between these two semantics.
We in general hope that any two semantics that purport to be ``for the same logic'' will be equi-valid.
One possible point of difference, which would be interesting to explore regardless the result, is the gap between computable reals and full reals: is there any formula that can only be satisfied or falsified when some variable is assigned a real that is not computable? 
If so, this would be quite an interesting case.
