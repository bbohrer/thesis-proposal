...
In general, the differences among formalizations mirror the differences among the underlying logics, so the unique features of our work include real analysis proofs, ODEs, and an explicit treatment of substitution.
...
\emph{Differential dynamic logic} (\dL)~\cite{DBLP:books/sp/Platzer18,DBLP:journals/jar/Platzer08,DBLP:journals/jar/Platzer17,DBLP:conf/lics/Platzer12b} is a dynamic logic for the formal verification of hybrid systems, which combine discrete transitions with differential equations to model CPSs.
...
In vanilla \dL, safety is usually expressed in the form $\phi \limply \dbox{\prepeat{(\ctrl;~\plant)}}{\psi}$ and the only forms of liveness which can be expressed are those with finitely-many player alterations, e.g.,
...
\end{figure*}
\begin{itemize}
\item The \ModelPlex tool, which is implemented as a feature of \KeYmaeraX, is invoked to synthesize a \emph{control monitor formula} and \emph{plant monitor formula}, which determine whether control decisions and physical evolutions respectively satisfy the assumptions made of them by the model
...
As discussed in \rref{sec:isabelle-fml}, this has been used to validate the \dL formalization for exhaustiveness.
...
\item The equivalent CakeML program ($\cmlsem{\cmldet{\sandbox}}{}$) is compiled to machine code in a verified compiler
\item The machine code ($\machsem{\cmlcompile{\cmldet{\sandbox}}}{}$)is linked against the users' implementation of sensors and actuators
\end{itemize}
Each step is provably a refinement (\rref{fig:proofchainoverview}) of the step before it, culminating that any execution of the machine-code running on the actual CPS corresponds to some execution of the source hybrid system.
...
It has static analyzers based on abstract interpretation such as NBac~\cite{DBLP:journals/fmsd/Jeannet03}, which eliminate many bugs in practice, but are fundamenteally conservative and also consider only the control code, not the plant.
...
\newcommand{\sshow}[3]{\text{show}~{#1}:{#2}~{#3}}
\newcommand{\shave}[4]{\text{have}~{#1}:{#2}~{#3}~{#4}}
\newcommand{\snote}[4]{\text{note}~{#1}:{#2}~{#3}~{#4}}
...
Furthermore, practical experience shows that modeling mistakes involving \emph{totality} are common: it is easy to write a controller with no transitions, making universal safety theorems \emph{vacuous}.
