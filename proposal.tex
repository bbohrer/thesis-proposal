...
My thesis is that these leaks can be sealed by placing the entire verification toolchain, from checking of safety proofs to runtime execution, on a common foundation in logic and programming languages.
That is: \emph{Constructive logic and functional programming enable end-to-end verification of CPS, connecting the synthesis of practical implementations down to bulletproof theoretical foundations}.
...
\paragraph{The Cast.}
Each approach listed above will surely claim to be practical, but practicality is a matter of perspective.
In this thesis, we arrive at practicality through the synthesis of three perspectives, represented by three characters: the Logician, the Engineer, and the Logic-User.
Through this synthesis we also show that no prior work addresses the needs of all three characters at once.

The Logician follows in the formalist tradition of David Hilbert: to know something is to have a proof of it, to have a proof is to have a machine-checkable derivation in a sound, formal proof calculus, and to have a sound proof calculus is to have a formal semantics against which the rules have a formal proof of soundness.
...
Even bugs in proof calculi, while less common, have been uncovered through careful verification.
While it cannot be known for sure whether fixing such bugs has ever prevented bugs in production, the Logician certainly seems justified in their paranoia.

The Engineer, however, may be shocked not only at the extent of paranoia but by the Logician's naivete.
The Engineer is tasked with designing, building, and delivering a production CPS under time and budget constraints.
The Engineer is no stranger to formal models, rather they are comfortable with highly complex models which completely confound the Logician's latest proof technique.
They are a stranger only to the Logician's infinite patience: any technique which cannot show concrete safety benefits on realistic systems in a short timeframe will ultimately be discarded.
To the Engineer, it is infinitely better to find one bug in the implementation than to eliminate every bug in an ideal model, because bugs in an ideal model might not translate to bugs in the implementation.

The Logic-User is oft-forgotten, and sits between the Logician and Engineer on the spectrum from theory to practice.
The Logic-User (sometimes called the Proof Engineer), is the person tasking with employing verification tools at scale.
...
As shown in \rref{tab:approach-comparison}, no approach satisfies all characters.
...
The Logic-User is not truly satisfied with any approach, but domain-specific logics like \dL come closest: a program-like syntax makes it possible to avoid more modeling mistakes than with automata, and proof is not as laborious as with GPITP.
...
\draw (-5, 2) node[rectangle,rounded corners,fill=green!20,draw,text depth=0.7in,text width=1.2in,text centered] (logic) {Foundations};
...
\draw (5, 2) node[rectangle,rounded corners,fill=green!20,draw,text depth=0.7in,text width=0.7in,text centered] (engine) {Artifacts};
...
\draw (1.45,-2) -- (0,-2.7);
\draw (0, -3.5) node[rectangle,rounded corners,fill=green!20,draw,text depth=0.85in,text width=0.8in,text centered] (luser) {Usability};
...
In the proposed work, we show that we can better meet the needs of the Engineer and Logic-User by developing a generalization of \dL called Constructive Differential Game Logic (\CdGL), which supports hybrid systems with adversarial dynamics (hybrid games) and constructive reasoning.
We first develop \CdGL, develop its foundations, and show that it leads to simpler models with less modeling errors (\rref{ch:cdgl}), then show (\rref{ch:proofplex}) that \CdGL supports synthesis of both controllers and monitors in the general case, in contrast to the state-of-the-art.

At last, we arrive at a concise thesis statement for the thesis as a whole:
\begin{framed}
  \begin{quote}
Constructive Differential Game Logic enables practical, end-to-end verification of cyber-physical systems.
  \end{quote}
\end{framed}
This is not to say that the thesis is the first practical approach to CPS verification, nor the last step in making verification end-to-end.
The needs of a real Logician, Logic-User, and Engineer are complex and ever-changing: tomorrow's Logic-User might seek full proof automation, tomorrow's Engineer might demand guaranteed resilience to sensing and actuation errors, and tomorrow's Logician might want a logical treatment of phenomena like concurrency and stochasticity which are challenging to incorporate with CPS today.
The real-world drama of CPS also has characters which are not discussed in this thesis, like government regulatory agencies, end-users of the production CPS, and even other CPS's which might want to interact with ours.
That being said, \rref{tab:approach-comparison} shows that the works of the thesis together a practical, foundational, end-to-end approach that meets the needs of the major players in a way that prior approaches have not.
...
%
We share a common goal: formally verifying that theorem provers are correct.
%
However, the foundations of \dL differ greatly from those of the other provers (\dL intimately deals with programs with differential equations), leading to substantially different proofs (in our case, intricate proofs about real analysis and substitution).
...
\chapter{Ongoing Work: \CdGL: Constructive Differential Game Logic}
\label{ch:cdgl}
...
For the programmer, the CPS has not been proven correct unless they know the monitor will be satisfied!
...
An intuitive notion of safety requires that at least one guard is satisfied in each state (the controller is \emph{total}), which is often false when models are not written carefully.
...
\CdGL is not only a generalization of  \dGL and \dL, but also of (Harel's) First-order Dynamic Logic and (Parikh's) Game Logic, which itself generalizes (Pratt's) PDL and (Peleg's) Concurrent Dynamic Logic.
...
%TODO: Reread nerode and his semantics
...
Like classical Kripke semantics, their intuitionistic counterparts are parameterized over worlds, except the worlds represent what is currently \emph{known} about the world.
...
Among intuitionistic dynamic logics, dynamic-epistemic logic (DEL)~\cite{DBLP:journals/logcom/FrittellaGKPS16} possesses a well-understood proof-theoretic semantics, but this semantics does not tell the whole story, since it neither exposes the underlying computational interpretation nor has its relation to a model-theoretic semantics been explored.
...
Intuitionistic Kripke semantics have also been given for a fragment of PDL~\cite{DBLP:journals/fuin/Celani01}, specifically multimodal System K with an iteration modality.
...
The benefits of this presentation are that it's clear Demon's strategy is being treated extensionally, i.e., Angel's strategy never gets to inspect Demon's strategy, and Angel's strategy works even if Demon employs noncomputable trickery.
...
Unlike many constructive logics, we assume that we are entitled to inspect the global state in a proof, just that we are not entitled to any magical, noncomputable inspections.
The discriminator $a$ allows us to capture the notion of which inspections are considered computable.
...
Lastly, union and complement do not correspond clearly to standard proof methods for semantically proving implications, while our awkward-looking definition is quite simple for use in semantic proofs.
...
We only consider a strategy to be well-formed if every $A \cup_\aa B$ satisfies the side conditions that $\aa(\om)=0$ implies $\om \in A$ and $\aa(\om)=1$ for all $\om$. 
...
%TODO: Constraint to just Y <=  [[Gamma]]
...
Unlike Angel, Demon is entitled to use noncomputable magic in its strategy, so $\dstrategyforR[\alpha\cup\beta]{\aa}{X}$ and $\dstrategyforR[\prepeat{\alpha}]{\aa}{X}$ use classical unions, not constructive unions.
...
\newcommand{\ebcons}[2]{[#1,#2]}
\newcommand{\eCons}[2]{\{#1,#2\}}
\newcommand{\etcons}[2]{\econs{#1}{#2}}
\newcommand{\einjL}[1]{\ell \cdot #1}
\newcommand{\einjR}[1]{r \cdot #1}
\newcommand{\ecase}[3]{\textrm{case }#1\textrm{ of }\ell.~#2~|~r.~#3}
\newcommand{\edinjL}[1]{\langle\ell \cdot #1\rangle}
\newcommand{\edinjR}[1]{\langle r \cdot #1\rangle}
\newcommand{\edcase}[3]{\langle\langle\textrm{case}\rangle\ #1\textrm{ of }\ell.~#2~|~r.~#3\rangle}
\newcommand{\eInjL}[1]{\{\ell \cdot #1\}}
\newcommand{\eInjR}[1]{\{r \cdot #1\}}
\newcommand{\eCase}[3]{\{\textrm{case}\}\ #1\textrm{ of }\ell.~#2~|~r.~#3}
\newcommand{\erep}[3]{{#1}\textrm{ rep }#3.~{#2}}
\newcommand{\eapp}[2]{#1\ #2}
\newcommand{\elam}[2]{\lambda x:#1.~ #2}
...
\newcommand{\eSeq}[1]{\{\iota~#1\}}
\newcommand{\eSeqinv}[1]{\{\iota~{#1}^{-1}\}}
\newcommand{\ebswap}[1]{[\textrm{swap }#1]}
\newcommand{\edswap}[1]{\langle\textrm{swap }#1\rangle}
\newcommand{\eSwap}[1]{\{\textrm{swap }#1\}}
\newcommand{\emon}[3]{{#1} {\circ_{#3}} {#2}}
\newcommand{\eQEpos}{\textrm{QE}_{pos}}
\newcommand{\eQEneg}{\textrm{QE}_{neg}}
\newcommand{\esplit}[1]{\textrm{split }#1}
\newcommand{\edroll}[1]{\langle\textrm{roll}\rangle\ #1}
\newcommand{\edunroll}[1]{\langle\textrm{unroll}\rangle\ #1}
\newcommand{\ebroll}[1]{[\textrm{roll}]\ {#1}}
\newcommand{\ebunroll}[1]{[\textrm{unroll}]\ #1}
\newcommand{\eRoll}[1]{\{\textrm{roll}\}\ #1}
\newcommand{\eUnroll}[1]{\{\textrm{unroll}\}\ #1}
\newcommand{\edasgn}[1]{\langle{:=}\ #1\rangle}
\newcommand{\edasgninv}[1]{\langle{:=}^{-1}\ #1\rangle}
\newcommand{\ebasgn}[1]{[{:=}\ #1]}
\newcommand{\ebasgninv}[1]{[{:=}^{-1}\ #1]}
\newcommand{\eAsgn}[1]{\{{:=}\ #1\}}
\newcommand{\eAsgninv}[1]{\{{:=}^{-1}\ #1\}}
\newcommand{\easgneq}[3]{#1\text{ was }#2\text{ in }#3}
\newcommand{\eunpack}[2]{\textrm{unpack}(#1,x y.~#2)}
\newcommand{\ewhile}[2]{\textrm{while}(#1 > 0)\{#2\}}
\newcommand{\eloopelim}[3]{(#1, x.~{#2}^{#3})}
\newcommand{\efp}[3]{\textit{FP}(#1, x.~#2, y.~#3)}
\newcommand{\G}{\Gamma}
...
This is the case because the assignment rule cannot always be applied left-to-right, and monotonicity allows applying it right-to-left when needed.
...
\end{lemma}
%TODO': Need to fix loop induction because substitution lemma fails
...
This solution is also natural because proof reductions for differential equations and quantifier elimination, respectively, naturally correspond to integrals and real-algebraic numbers.
...
We say a formula $\phi$ is $\alpha$-controllable in $\Gamma$ if there exists a computable function $f:\allstate\mto\allpt$ such that in every $\omega \in \fint{\Gamma}{},$ we have $\proves{\Gamma}{f(\omega)}{\ddiamond{\alpha}{\phi}}$ and additionally $\alpha$ is decideable (i.e., deterministic, with all branches computable) within $\Gamma$.
