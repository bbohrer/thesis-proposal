...
Stefan Mitsch \\
Frank Pfenning \\
Tobias Nipkow xor Gordon Plotkin
}
...
That being said, \rref{tab:approach-comparison} shows that the works of the thesis together a practical, foundational, end-to-end approach that meets the needs of the major players in a way prior approaches have not.
...
{\Huge TODOs}:
\begin{itemize}
\item Decide whether to overload conjunctionlike, disjunctionlike proof terms or make distinct so typing will be unique
\item Decide whether to reuse conjunction, disjunction as normal forms of loops vs.\ introduce roll and unroll operator, which would enable true NF theorem instead of WHNF
...
\item Prove theorems and lemmas: Substitution, EP, DP, Progress+Preservation, WHNF, Soundness, Unicity (if true)
\end{itemize}
...
\end{itemize}
\end{lemma}
However, there are reasons to be suspicious of the alternative rule.
This use of renaming can make the theory a nightmare to get right, since a so-called ``state'' now incorporates an entire execution trace.
Renaming the context should especially raise alarm bells, since $\vdash$ is no longer natural in $\Gamma$ and structural properties may start to fail.
It is also unclear, for example, whether subject reduction will hold for arbitrary contexts, perhaps those containing functions.
So I propose sticking with the former calculus and theorem, and this operational semantics is written assuming as such.
...
%\newcommand{\eunpack}[2]{\textrm{unpack}(#1,x y.~#2)}
%\newcommand{\ewhile}[2]{\textrm{while}(#1 > 0)\{#2\}}
%\newcommand{\eloopelim}[3]{(#1,{#2}^{#3})}
%\newcommand{\efp}[3]{\textit{FP}(#1,#2,#3)}
%\newcommand{\einjL}[1]{\ell \cdot #1}
%\newcommand{\einjR}[1]{r \cdot #1}
%\newcommand{\ecase}[3]{\textrm{case }#1\textrm{ of }\ell.~#2~|~r.~#3}
%\newcommand{\eQEpos}{QE_{pos}}
%\newcommand{\eQEneg}{QE_{neg}}
...
If $\proves{\Gamma}{M}{\phi},$ then either $M$ is (weak-head) normal or $M \stepsto N$ for some $N$.
\end{lemma}
