...
%\end{dedication}
\begin{abstract}
Cyber-physical systems (CPSs) combining discrete control and continuous physical dynamics are pervasive in modern society: 
examples include driver assistance in cars, industrial robotics, airborne collision avoidance systems, and the electrical grid.
Many of these systems are safety-critical  because they operate in close proximity to humans.
Formal safety verification of these systems is important because it is a key tool for attaining the strongest possible safety guarantees.

\emph{Hybrid systems} models, in particular, are a successful formalism for CPS.
Hybrid systems theorem-proving in differential dynamic logic (\dL) and its generalization differential \emph{game} logic (\dGL) are notable for strong logical foundations and successful application in  case studies using the theorem provers \KeYmaera and \KeYmaeraX.
However, safety verification of models does not imply safety of \emph{implementations}, which might not be faithful to the model.
Further still, a formal proof means nothing if the proof-checking software is \emph{unsound}.

This thesis addresses implementation and soundness gaps by using constructive logic and programming languages as the foundation of an end-to-end verification toolchain.
...
\CdGL enables synthesis of implementations with bulletproof theoretical foundations.
Logic is the keystone of the end-to-end connection from high-level proofs and foundations to implementations.
Our pursuit of practical proving includes innovations in the proof scripting language itself.

\CdGL proofs, in contrast to \dGL, are suitable for synthesizing \emph{controllers} which determine safe actions for a CPS and \emph{monitors} which check the compliance of the external environment with model assumptions.
The synthesized code is automatically proven correct down to machine-code level.
The foundations are also strengthened by our formalization of \dL's soundness in \Isabelle, allowing proofs in the fragment \dL to be exported and rechecked.
We evaluate the toolchain on a 2D robot which follows arcs.
The model and implementation cross-validate each other: monitors catch incorrect code and assumptions, while testing with monitors enabled allows us to assess the realism of the model.
\end{abstract}
...
% Connection column? weak weak weak strong strong
\begin{table}[tbh]
  \centering
...
\draw (5, 0.2) node[rectangle,rounded corners,fill=\cprop,draw,thin,minimum height=0.3cm] (proofscript) {Proof Script};
...
The already-completed works (shown in green) in this thesis connect implementation artifacts to foundations by way of \dL:
...
\rref{ch:logical-foundations} then satisfies the Logician by putting \dL (as implemented in \KeYmaeraX) on firm footing in \Isabelle (\rref{sec:isabelle-fml}).
\rref{sec:definite-description} generalizes the foundations of \dL to survive future development.
\rref{ch:cgl} develops a constructive game logic (\CGL) for discrete games.
A paper submission on \CGL has been made, but the work could be revised (thus shown in orange) during resubmission.
The proposed work (in yellow) extends our logical foundation by developing \CdGL (Constructive \dGL) and Kaisar, a principled language for \CdGL proofs.
...
These additions meet the practical needs of the Engineer and Logic-User while providing a higher-level input language for the \VeriPhy pipeline and generating an additional output: verified controllers.
% on one end to high-level user proofs scripts and rounds out the other end by providing verified controllers, resulting in a stronger safety guarantee than prior versions of \VeriPhy.
...
Constructive Differential Game Logic (\CdGL) enables practical, end-to-end verification of cyber-physical systems.
  \end{quote}
\end{framed}
In \rref{sec:relwork}, we will compare the \VeriPhy approach in detail with other works that strive for an end-to-end philosophy.
Our take on end-to-end verification is that implementation-level guarantees must come by formally connecting implementation to models, else the Logician will not accept the system as verified.
For practicality, we transport guarantees to implementation level automatically, with machine-code artifacts that the Engineer can use.
In \rref{sec:ground-robotics}, our 2D ground robotics case study serves to show our approach scales to non-trivial scenarios.
The Logic-User must be able to perform their proofs in a high-level, productive language.
Viewed through this lens, \rref{tab:approach-comparison} shows that the \VeriPhy approach, with its strong foundation in logic, marries practical end-to-end verification foundation with rigorous foundations on a level far beyond the state-of-the-art.

\section{Related Work}
% TODO: Revise all related works for Frank, tobias, etc
\label{sec:relwork}
...
In general, the differences among formalizations mirror the differences among the underlying logics, so the unique features of our work include real analysis proofs, ordinary differential equations (ODEs), and an explicit treatment of substitution.
...
%A recent work~\cite{DBLP:conf/itp/ImmlerRW19} takes the novel approach of running
...
% Interchange DBLP:conf/hybrid/BakBJ15  DBLP:conf/hybrid/PintoCPS06
%
...
% information flow: DBLP:conf/hicons/PrabhakarK13
...
% coq: ROSCoq~\cite{DBLP:conf/itp/AnandK15} and VeriDrone~\cite{DBLP:conf/cpsweek/MalechaRAL16} allow CPS specification, implementation, verification, and code generation in Coq.
...
\emph{Differential dynamic logic} (\dL)~\cite{DBLP:books/sp/Platzer18,DBLP:journals/jar/Platzer08,DBLP:journals/jar/Platzer17,DBLP:conf/lics/Platzer12b} is a dynamic logic for the formal verification of hybrid systems, which combine discrete transitions with differential equations (ODE's) to model CPS's.
...
While some thesis chapters (\rref{ch:logical-foundations}, \rref{sec:veriphy}) employ \dL, we present full \dGL since 
\begin{inparaenum}[i)]
  \item \rref{ch:cgl} (completed) relies on games,
...
We recall the syntax and informal meaning of \dGL here, then recall the winning-region semantics formally in \rref{sec:dgl-semantics}.
...
It is worth noting that with the exception of \cite{DBLP:conf/cade/QueselP12}, prior case studies ~\cite{DBLP:conf/emsoft/JeanninGKGSZP15,DBLP:conf/fm/LoosPN11,DBLP:journals/ijrr/MitschGVP17,DBLP:conf/hybrid/PlatzerQ08} used vanilla \dL for hybrid \emph{systems} in \KeYmaeraX (\rref{sec:veriphy},\rref{sec:ground-robotics}).
...
This is because the \emph{strategy} by which acceleration is chosen must be expressed in the model.
In this case, the strategy has three cases:
...
While many basic axioms are the same in \dL and \dGL, some are necessarily different because \dGL is subregular: for example, Kripke's axiom K fails in \dGL and is in practice subsumed by rule \irref{dglMon}.
...
{
\linferenceRule[equiv]{\phi(\theta)}{\ddiamond{\humod{x}{\theta}}{\phi(x)}}
}{}
\cinferenceRule[dglrandom|{$\langle{:}*\rangle$}]{}
{
\linferenceRule[equiv]{\lexists{x}{\phi(x)}}{\ddiamond{\prandom{x}}{\phi(x)}}
}{}
%\begin{figure}
%  \centering
%  \begin{calculuscollections}{\columnwidth}
...
{
\linferenceRule[equiv]{\lexists{t{\geq}0}{\ddiamond{\humod{x}{y(t)}}{\phi}}}{\ddiamond{\pevolve{\D{x}=\theta}}{\phi}}
}{}
...
Axiom \irref{dglfixpoint} says repetition $\prepeat{\alpha}$ is a fixed point of $\alpha$.
...
\end{figure*}
\begin{itemize}
\item The \ModelPlex~\cite{DBLP:journals/fmsd/MitschP16} tool, which is implemented as a feature of \KeYmaeraX, is invoked to synthesize a \emph{control monitor formula} and \emph{plant monitor formula}, which determine whether control decisions and physical evolutions respectively satisfy the assumptions made of them by the model.
\item The monitors are combined with a proven-safe \emph{fallback controller} (which is provided in the input model) to form a \emph{sandbox controller}.
   The sandbox controller applies control decisions provided from an untrusted source (henceforth called the \emph{external controller}) whenever those decisions satisfy the controller monitor,
...
As discussed in \rref{sec:isabelle-fml}, this has also been used to validate the \dL formalization for exhaustiveness.
\item The exact real arithmetic from hybrid systems semantics ($\iaccess[\cdot]{\I}$) is conservatively recast as fixed-point interval arithmetic ($\wsem{\cdot}{}$), and this recasting is formally proven in Isabelle to be sound.
\item The resulting program is automatically proven equivalent (in HOL4) to a CakeML~\cite{DBLP:journals/jar/KumarAMO16} program.
The heart of the proof says that sensing and actuation in the model can be modeled by a CakeML state machine~\cite{DBLP:conf/cade/HoAKMTN18}.
\item The equivalent CakeML program ($\cmlsem{\cmldet{\sandbox}}{}$) is compiled to machine code in the CakeML verified compiler
\item The machine code ($\machsem{\cmlcompile{\cmldet{\sandbox}}}{}$) is linked against the user's trusted implementation of sensors and actuators
\end{itemize}
Each step is provably a refinement (\rref{fig:proofchainoverview}) of the step before it, culminating in the fact that any execution of the machine-code running on the actual CPS corresponds to some execution of the source hybrid system.
...
The differing brackets at each level represent that states at each level have differing types, e.g. reals vs.\ intervals, and these types are converted accordingly in each refinement theorem.
The CakeML sandbox ($\cmldet{\sandbox}$) is generated by importing the monitor conditions and fallback used in the \dL $\sandbox$.
...
The extra factor of $(1 + \abs{k}\veps)^2$ in formula $\admiss$ accounts for the fact that an arc along the inner side of the annulus is shorter than one along the outside (\rref{fig:circlestaging}).
...
\newcommand{\dinline}[2]{\textsf{inline}_{\dbox{#1}{}}{(#2)}}
\newcommand{\ainline}[2]{\textsf{inline}_{\ddiamond{#1}{}}{(#2)}}
...
This promotes a proof style where controller proofs are Angelic ($\ddiamond{\alpha}{\phi}$)  while plant proofs are Demonic ($\dbox{\alpha}{\phi}$).
In contrast, a systems safety proof treats both the controller and plant as Demonic.
If we recall the contrast between \rref{ex:driving-system} and \rref{ex:driving-game}, we see that Demonic controller models are typically much more complicated than their Angelic counterparts, because they explicitly give an envelope of safe control choices, which is left implicit in the Angelic model.
In non-trivial models such as \rref{ch:end-to-end-v}, this control envelope is a significant contributor to overall model complexity.
Furthermore, practical experience shows that modeling mistakes involving \emph{totality} are common: it is easy to write a controller which has nonexhaustive case analyses, making universal safety theorems \emph{vacuous}.
...
Suppose $c \equiv 0 ~(\textup{mod}~ 4),$  Angel controls the iteration of the loop, and Demon acts first.
...
This implies Angel wins the game because Demon will violate the rules once $c=0$ and no move is valid..
...
Suppose $c \not\equiv 1 ~(\textup{mod}~ 4)$ initially, Angel controls the iteration of the loop, and Angel acts first.
...
%TODO: Careful to introduce notations
\section{Semantics}
\label{sec:rel-sem}
...
\end{definition}
%\rzFst{\aa} \alternative \rzSnd{\ab} \alternative
...
Rules \irref{dchoiceIL} and \irref{dchoiceIR} say that a diamond choice $\ddiamond{\alpha\cup\beta}{\phi}$ is introduced by picking between $\alpha$ or $\beta,$ then proving the corresponding branch.
...
%  If $\phi$ is valid, then $\phi$ is Kripke-valid.
%\end{theorem}
...
{
\linferenceRule[equiv]{\lexists{t{\geq}0}{\ddiamond{\humod{x}{y(t)}}{\phi}}}{\ddiamond{\pevolve{\D{x}=f}}{\phi}}
}{}
\cinferenceRule[dglweak|DW]{}
{
\linferenceRule[equiv]{\dbox{\pevolvein{\D{x}=f}{\psi}}{\phi}}{\dbox{\pevolvein{\D{x}=f}{\psi}}{(\psi\limply\phi)}}
}{}
...
{
\linferenceRule[lpmi]{\dbox{\pevolvein{\D{x}=f}{\psi}}{\phi}}{\phi \land \dbox{\pevolvein{\D{x}=f}{\psi}}{\der{\phi}}}
}{}
\cinferenceRule[dglghost|DG]{}
{
\linferenceRule[equiv]{\lexists{y}{\dbox{\pevolvein{\D{x}=f,\D{y}=a(x)*y+b}{\psi}}{\phi}}}{\dbox{\pevolvein{\D{x}=f}{\psi}}{\phi}}
}{}
\cinferenceRule[dglconst|DS]{}
{
\linferenceRule[equiv]{(\lforall{t}{(\lforall{0{\leq}s{\leq}t}{\dbox{\humod{x}{x+cs}}{\psi}}) \limply \dbox{\humod{x}{x+ct}}{\phi}})}{\dbox{\pevolvein{\D{x}=c}{\psi}}{\phi}}
}{}
\cinferenceRule[dglvariant|DV]{}
{\linferenceRule[lpmi]{\ddiamond{\pevolve{\D{x}=h}}{f \geq g}}{\lexists{\veps>0}{(\dbox{\pevolve{\D{x}=h}}{(f \leq g \limply \D{f} \geq \D{g}+\veps)})}}
}{}
\end{calculus}
\end{calculuscollections}
...
Validity of \irref{dglind} follows from the Mean-Value Theorem (a special case of Rolle's Theorem), which has been proven constructively~\cite{Krebbers+Spitters:lmcs:corn:2011,Krebbers+Spitters:lmcs:corn:2011}.
The classical soundness proofs of \irref{dglcut} and \irref{dglweak} are direct, and we expect the same for an appropriately-chosen semantics.
...
\begin{theorem}[Angel Robot Winnability]
\label{thm:angel-wins}
The following \CdGL formula is valid:
\begin{align*}
&\Avar{>}0 \land \Bvar{>}0 \land \Tvar{>}0 \land \veps{>}0 \land \linv \limply \\
...
\end{theorem}
Winnability subsumes liveness because it says there exist control decisions which reach the goal, given arbitrary Demonic control of the plant duration.
We propose to generalize \rref{thm:angel-wins} slightly, yielding a single theorem which subsumes both \rref{thm:liveness} (liveness) and \rref{thm:safe} (safety) in one go.


%In this form, \rref{thm:angel-wins} corresponds to liveness, \rref{thm:liveness}.
\begin{theorem}[Generalized Angel Robot Winnability]
...
%, and give examples of when nominal reasoning in program proof and modeling may be useful.
...
The program construct $?(x:\phi)$ can also function like the Isar ``assume'' keyword for introducing assumptions since  the implication $\phi \rightarrow \psi$ and box test $[?\phi]\psi$ are equivalent by axiom \irref{dgltest}.
