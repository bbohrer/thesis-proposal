...
\rref{ch:cgl} develops a constructive game logic for discrete games.
...
GPITP's such as Coq and Isabelle have also been used to verify hybrid systems~\cite{DBLP:conf/cpp/Rouhling18}, related models of CPS's~\cite{DBLP:conf/atva/RizaldiISA18,DBLP:conf/ifm/RizaldiKHFIAHN17,chan2016formal}, and related topics such as geometry~\cite{DBLP:conf/cpp/AffeldtC17} and dynamical systems~\cite{DBLP:conf/itp/CohenR17}.
GPITP's provide access to large libraries of general-purpose mathematics, but do not provide a tailored experience.
...
\end{align*}
In a Demonic loop, Demon controls \emph{only} the duration of the loop, then returns control to Angel during the body.
...
The game $\alpha_s$ is a model of adversarial driving where the opponent Demon controls the duration (${}^\times$).
We will write $\alpha_l$ to denote the counterpart where Angel controls the duration.
...
The remainder of the game is a \emph{demonic loop} $\cdot^\times$ where the Demon player controls the duration.
...
A typical game model has shape $\prepeat{(\pdual{\ctrl};~\plant)},$ where the turn-taking (or dual) operator $\pdual{\alpha}$ has the effect of switching players.
...
\dstrategyfor[\prandom{x}]{X}       &= \{ \om \in \allstate~|~\subst[\om]{r}{\tint{\theta}{\om}}\text{ for all }r \in \mathbb{R} \in X\} \\
...
\section{Status}
The work presented in this chapter is under submission to POPL'20~\cite{poplcgl}, with a likely rejection based on mixed reviews.
...
{
\linferenceRule[equiv]{\lexists{t{\geq}0}{\ddiamond{\humod{x}{y(t)}}{\phi}}}{\ddiamond{\pevolvein{\D{x}=f}{\phi}}}
}{}
\cinferenceRule[dglweak|DW]{}
{
\linferenceRule[equiv]{\dbox{\pevolvein{\D{x}=f}{\ivr}}{\phi}}{\dbox{\pevolvein{\D{x}=f}{\ivr}}{(\ivr\limply\phi)}}
}{}
...
Forward style proof terms correspond loosely to the keywords \texttt{of} and \texttt{OF} in Isar, or to a minimal proof term language with modens ponens in logic or type theory.
...
Since it is unlikely that all components will be finished by then, and because timelines are only estimates, I have listed several conference options for each work.
I intend to defend approximately August 2020.

Total: 12.5 months
\begin{itemize}
\item 2 weeks Resubmit \CGL somewhere (e.g., LICS, FSCD, FOSSACS, CSL, ICALP)
