...
The preorder $\leq$ on intuitionisic worlds is also given concretely: $X \leq Y \equiv X \superseteq Y$: that is, if $Y$ is a subset of $X$ then it contains more information than $Y$ because it rules out all of the states that $Y$ does, plus some more.
...
Therefore we will choose $\tint{\theta}{\om} : 2^{\mathbb{R}}$: the value of a term in some state is the set of all values it might currently take on.
The semantics are defined by:
\begin{align*}
  \tint{q}{\om} &= \{q\}\\
  \tint{x}{\om} &= \{\om(x)\}\\
  \tint{\theta + \eta}{\om} &= \tint{\theta}{\om} \oplus \tint{\eta}{\om} = \{r + s~|~r \in \tint{\theta}{\om}\text{ and }s \in \tint{\eta}{\om}\}\\
  \tint{\theta \cdot \eta}{\om} &= \tint{\theta}{\om} \otimes \tint{\eta}{\om} = \{r s~|~r \in \tint{\theta}{\om}\text{ and }s \in \tint{\eta}{\om}\}\\
  \tint{\der{\theta}}{\om} &= \sum_{x\in\allvars}\frac{\partial\tint{\theta}{\om}}{\partial x}\om(\D{x})\text{ for deterministic }\theta \\ % in an open ball around omega
...
\begin{align*}
  ...
  \fint{\bot}{X} &\lequiv \emptyset\\
  (\fint{\theta \geq \eta}{X}\text{ true})&  \lequiv \text{for all }r\in \tint{\theta}{\om}, s\in\tint{\nu}, \om,\nu \in X, r \geq s\\
  (\fint{\theta \geq \eta}{X}\text{ false})& \lequiv \text{for all }r\in \tint{\theta}{\om}, s\in\tint{\nu}, \om,\nu \in X, r < s\\
  \fint{\exists{x}{\phi}}{X} &\lequiv \text{exists }F, \fint{\phi}{\{\subst[\om]{x}{r}~|~\om in X, \subst[\om]{x}{r} \in F\}}\\
...
\begin{align*}
  ...
  (X,Y) \in \pint{\pevolvein{\D{x}=\theta}{\ivr}}\text{ iff }& \text{ exists }F, Y = \{\nu~|~\om \in X, t \in \mathbb{R}_{\geq0}, \varphi_\om:[0,t]\to\allstate.~\varphi_\om(0)=\om\text{ on }\{x,\D{x}\}^C,
   \frac{\partial \varphi_\om(t)(x)}{\partial t}= \varphi_\om(t)(\D{x})\text{ and } \{\varphi_\om(t)(x)\}=\tint{\theta}{\varphi(\varphi(s))}\text{ and }\fint{\ivr}{\{\varphi_\mu(t)~|~\mu\in X}}\}
...
\begin{itemize}
\item Read Fischer Servi cited in Degen + Werner (intuitionistic-base.pdf)
