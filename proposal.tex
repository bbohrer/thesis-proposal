...
% TODO: Why is this the right question, compare to first model, compare systems vs. games, compare to initial mode cite, motivate in overall intro too.
% sec:veriphy
...
%TODO: Reduce down, seems unnecessary
\begin{table*}
...
%TODO: MAJOR: Add Kaisar section
%TODO: MAJOR: Change to done work, add section on proposed d extension
...
%TODO: Condense, update from POPL
\section{Related Work}
...
That is, the following \CGL formula is valid (true in every state): %TODO: not quite what valid means
...
%TODO: discuss duality axioms around here
%This insight is revealed most clearly for \GL, which generalizes \DL with a duality operator $\pdual{\alpha}$ whose effect is to pass control between the players and thereby alternate between the modalities: $\dbox{\pdual{\alpha}}{\phi} \lequiv \ddiamond{\alpha}{\phi}$ and vice versa.
%The dual region of $X$ is written $\sdual{X},$ and is defined by $\sff \in \sdual{X}$ iff $\stt \in X,$ as well as $\stt \in \sdual{X}$ iff $\sff \in X,$ and $\sdual{X}=X$ otherwise.
...
%TODO resolve the \bigwedge explicitly in this definition of validity. Or at least say that it'll divvy up the realizer so that there's one for each conjunct, never mind associativity.


%TODO: Polish importance
...
% TODO: Reduce and then say more arith is good question for full thesis then cite Lombardi 
...
%TODO: Talk about why arbitrary computable functions were used in \CGL anyway
\section{\CdGL Theory}
...
%TODO: Update to correct rule
\cinferenceRule[dglvariant|DV]{}
{
\linferenceRule[lpmi]{\ddiamond{\pevolve{\D{x}=f}}{g>c()}}{\lexists{d>0}{\dbox{\D{x}=f}{g\geq d}}}
}{}
\end{calculus}
\end{calculuscollections}
...
Picard-Lindel\"{o}f is also the basis of validity for \irref{dglghost}, which says it is sound to extend a system of differential equations with a new continuous variable, so long as the new equation is linear.
%TODO: Cite tyk lics18
...
% TODO: Make fonts consistent A B T v need fix
\begin{align*}
\plant\equiv \{&\D{\xgvar}=-\vvar~\kvar~\ygvar,~\D{\ygvar}=\vvar~(\kvar~\xgvar-1),~\D{\vvar}=\avar,~\D{\tvar}=1 \\
 &\&~\tvar\leq \Tvar ~\land~ \vvar \geq 0\} %  ~\land~ yg > 0
\end{align*}
...
%TODO: Talk about what right theorems in games are vs liveness in systems
...
We also propose to prove completeness against the \CdGL calculus.

%TODO: Maybe a paragraph about implementation

\section{Related Work}
\label{sec:kaisar-relwork}
%TODO: Talk about VCGs, Coq, Dafny, VDM, old(), Apt, see arxiv paper for more
...
``Mizar modes'' have been used to implement structured proofs to provers without extending the core.
...
A number of verification tools ~\cite{KeYBook,DBLP:conf/cade/FultonMQVP15,DBLP:conf/lpar/Leino10,this-is-boogie-2-2,Barnett2005,DBLP:conf/fosad/LeinoMS09,DBLP:books/daglib/p/LeavensBR99} offer ad-hoc historical reference constructs without theoretical justification which can reference only the initial program state.
...
%VeriML has a different focus: promoting prover extensibility while providing tactic efficiency with staging.
%Their metatheory validates the safety of staging, while our metatheory validates the correctness and generality of reasoning across states with nominals.
%In our metatheory, \dLN{} formulas provide a clean specification for the nominal terms of Kaisar.
%TODO: Discuss plans for pattern-matching in Kaisar v2
...
%TODO: Rewrite in terms of characters
...
%Thus, a constructive games logic (\CGL) is the optimal foundation to resolve the concerns of all three.
%TODO: Condense
%\item
